...
<TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
  <TBODY>
    <TR>
      <TD colspan="2" vAlign=top class="header-bk"><H3>Gest&atilde;o de Produtos</H3></TD>
    </TR>
    <TR>
      <TD colspan="2" vAlign=top height="5"></TD>
    </TR>
    <TR>
      <TD width="36" vAlign=top><img src="<?=$staticvars['site_path'];?>/modules/produtos/images/puzzle-pieces.gif" alt="" width="32" height="26" align="baseline"><BR></TD>
      <TD vAlign=top>Este m&oacute;dulo permite efectuar a gest&atilde;o de todo o stock de produtos da sua empresa. Aqui pode adicionar produtos na p&aacute;gina web, apagar, modificar assim como retirar tempor&aacute;riamente um produto.</TD>
    </TR>
  </TBODY>
</TABLE>
<br>
...
if(isset($_POST['produto_submit'])):
		include($staticvars['local_root'].'modules/produtos/update_db/db_management.php');
...
if(isset($_GET['edit'])):
	...
	$test=$db->getquery("select cod_iva from produtos_iva");
	$test2=$db->getquery("select cod_desconto from produtos_desconto");
	$test3=$db->getquery("select cod_categoria from produtos_categorias");
...
<table width="100%" border="0">
  <tr valign="top">
    <td width="40"><img src="<?=$staticvars['site_path'];?>/modules/produtos/images/warning.png" alt="warning" />&nbsp;</td>
...
<?php
if($cat<>0):
	$prods=$db->getquery("select cod_produto, titulo, short_descricao, imagem from produtos where cod_categoria='".$cat."'");
	if($prods[0][0]<>''):
		echo '<strong>Listagem dos produtos existentes nesta categoria</strong><br>';
		echo '<table width="100%" border="0">'.chr(13);
			for ($jy=0;$jy<count($prods);$jy++):
					if ($prods[$jy][3]=='' or $prods[$jy][3]=='no_img.jpg' or !is_file($upload_directory.'/produtos/images/'.$prod[$i][5])):	
						$image='<img src="'.$staticvars['site_path'].'/modules/produtos/images/no_img.jpg"  width="100" border="0">';
					else:
						$image='<img src="'.$upload_path.'/produtos/images/'.$prods[$jy][3].'" width="100" border="0">';
					endif;
		...
		<table border="0" align="center">
		  <tr valign="top">
			<td width="40"><img src="<?=$staticvars['site_path'];?>/modules/produtos/images/info.gif" alt="info" />&nbsp;</td>
			<td valign="middle"><p>Não há produtos nesta categoria</p>
			</td>
		  </tr>
	  </table>
		<?php
	...
	<table border="0" align="center">
	  <tr valign="top">
		<td width="40"><img src="<?=$staticvars['site_path'];?>/modules/produtos/images/info.gif" alt="info" />&nbsp;</td>
		<td valign="middle"><p>Seleccione uma categoria para poder listar os produtos</p>
	    </td>
	  </tr>
  </table>
	<?php
...
include($staticvars['local_root'].'kernel/staticvars.php');
$prod=$db->getquery("select cod_produto, titulo, descricao, prazo_entrega, preco, imagem, cod_iva, cod_desconto from produtos where active='?' or active='n'");
$address=strip_address("cod",$_SERVER['REQUEST_URI']);
if (@$prod[0][0]<>''):
	$total=count($prod);
	$lower=@$_GET['lower'];
	...
	$up=$upper;
	echo '<strong>Listagem dos produtos que não se encontram publicados</strong><br />';
	put_previous_next_page($lower,$upper,$total,$_SERVER['REQUEST_URI']);
	if ($up > ($total-1)):
		$up=($total-1);
	endif;
	for ($i=($lower-1);$i<=$up;$i++):
		if($prod[$i][5]<>'' or $prod[$i][5]<>'no_img.jpg' or !is_file($upload_directory.'/produtos/images/'.$prod[$i][5])):
			$img=$upload_path.'/produtos/images/'.$prod[$i][5];
		else:
			$img=$staticvars['site_path'].'/modules/produtos/images/no_img.jpg';
		endif;
		$desconto=$db->getquery("select valor from produtos_desconto where cod_desconto='".$prod[$i][7]."'");
		$iva=$db->getquery("select valor from produtos_iva where cod_iva='".$prod[$i][6]."'");
		?>
  ...
  &euro;</td>
	...
	echo '<table width="500" border="0"><tr><td align="center"><img src="'.$staticvars['site_path'].'/modules/produtos/images/warning.png" /><br>Todos os produtos encontram-se publicados.<br></td></tr></table>';
endif;
};
...
$email_options='';
if(isset($_GET['cod'])):
	$prod=$db->getquery("select titulo, descricao, short_descricao, imagem, preco, cod_desconto, cod_iva, stock, prazo_entrega, ref_produto, cod_categoria, active from produtos where cod_produto='".mysql_escape_string($_GET['cod'])."'");
	if($prod[0][0]<>''):
		if($prod[0][11]=='s'):
	...
	*/
    ...
    <tr>
      ...
      <td><select name="iva" id="iva" class="form_input">
			<?php
			$query=$db->getquery("select cod_iva, descricao,valor from produtos_iva");
			$selected=0;
			$option[0][0]='0';
			$option[0][1]=' ';
			if($query[0][0]<>''):
    ...
    <select name="desconto" id="desconto" class="form_input">
      <?php
	  	$option=array();
        $query=$db->getquery("select cod_desconto, descricao,valor from produtos_desconto");
        $selected=0;
        $option[0][0]='null';
        $option[0][1]='-----------------';
        if($query[0][0]<>''):
