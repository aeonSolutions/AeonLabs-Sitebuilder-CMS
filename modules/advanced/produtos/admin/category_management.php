...
if(isset($_POST['cat_name']) or isset($_POST['add_cat_name']) or isset($_POST['cat_code']) or isset($_POST['del_cat'])):
	...
	include($staticvars['local_root'].'modules/produtos/update_db/category_management.php');
endif;
$query=$db->getquery("select cod_categoria, nome from produtos_categorias where cod_categoria='".$cod_categoria."'");
if ($query[0][0]<>''):
	$cat_name=$query[0][1];
else:
	$cat_name='raiz';
...
<br />
<h3><img src="<?=$staticvars['site_path'].'/modules/produtos';?>/images/icone_gestao.gif" width="24" height="24" />Manuten&ccedil;&atilde;o de Categorias de Produtos</h3>
<br />
<br />
<TABLE width="100%" height="500" border="0" cellPadding="0" cellSpacing="0">
  <tr valign="top" height="10">
	<td align="center">
        ...
        <hr class="gradient">
		<font class="body_text">Ver sub-categorias existentes:&nbsp;</font><select size="1" name="cat" class="form_input">
			<?php
			$query=$db->getquery("select cod_categoria, nome from produtos_categorias where cod_sub_cat='".$cod_categoria."'");
			$option[0][0]='';
			$option[0][1]='-----------------';
			if($query[0][0]<>''):
		...
		<br />
		<br />
		<img src="<?=$staticvars['site_path'];?>/modules/produtos/images/add.gif" width="24" height="22" /><a class="body_text" href="<?=$_SERVER['REQUEST_URI'].'&type=add&cat='.$cod_categoria;?>" >Adicionar nova categoria dentro desta categoria</a>
		<hr class="gradient">
		</form>
	</td>
  </tr>
  ...
  <tr valign="top" height="100%">
	...
	<?php
	if ($cod_categoria<>''):
		$query=$db->getquery("select cod_categoria from produtos_categorias where cod_categoria='".$cod_categoria."'");
		if (isset($_GET['type'])):
			if($_GET['type']=='add'):
...
$query_a=$db->getquery("select cod_categoria, active,nome,descricao from produtos_categorias where cod_categoria='".$mod."'");
if($query_a[0][1]=='s'):
	$pub='Sim';
	$name='Desactivar';
	$value='nao_publicar';
...
<table border="0" height="50" align="center">
  <tr>
	...
	if ($mod<>''):
		...
		$i=0;
		$res[0][0]='';
		$query=$db->getquery("select cod_categoria, nome, cod_sub_cat from produtos_categorias where cod_categoria=".$ct);
    ...
    <td style="BORDER-RIGHT: lightsteelblue 1px solid; BORDER-TOP: lightsteelblue 1px solid; MARGIN: 0px; OVERFLOW: hidden; BORDER-LEFT: lightsteelblue 1px solid; BORDER-BOTTOM: lightsteelblue 1px solid;">
	<?php
	$query=$db->getquery("select cod_categoria, nome from produtos_categorias where cod_sub_cat='".$mod."' order by nome");
	if ($query[0][0]==''):
		?>
		...
		while ($i<count($query)):
			...
			<tr>
			<?php
			$query2=$db->getquery("select cod_categoria, nome from produtos_categorias where cod_sub_cat='".$query[$i][0]."' order by nome asc limit 0,7");
			if ($query2[0][0]<>''):
				?>
			...
			if (isset($query[$i+1])):
				$query3=$db->getquery("select cod_categoria, nome from produtos_categorias where cod_sub_cat='".$query[$i+1][0]."' order by nome asc limit 0,7");
				if ($query3[0][0]<>''):
					?>
					<td align="center" valign="top" width="50%"><div align="left">
					<?php
