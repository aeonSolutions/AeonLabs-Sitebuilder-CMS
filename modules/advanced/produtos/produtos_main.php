<?php
/*
File revision date: 12-mar-2008
*/
// $auth_type:
// 		Administrators
//		Guests
...
if (isset($_GET['cod'])): // 1 produto definido
	...
	$prod=$db->getquery("select titulo, descricao, short_descricao, imagem, preco, cod_desconto, cod_iva, stock, prazo_entrega, ref_produto, cod_categoria,cod_produto, catalogo active from produtos where cod_produto='".$cod."' and active='s'");
	if ($prod[0][0]<>''):
		list_on_product($staticvars,$cod,$prod,$pmo);
	else:
		general_product_listing($staticvars,$pmg);
	endif;
else: // listagem de produtos
	general_product_listing($staticvars,$pmg);
endif;

function list_on_product($staticvars,$cod,$prod,$pmo){
include($staticvars['local_root'].'kernel/staticvars.php');
if ($prod[0][12]<>'' and $prod[0][12]<>'no_cat' and is_file($upload_directory.'produtos/catalogo/'.$prod[0][12])):
	include($staticvars['local_root'].'kernel/initialize_download.php');
	$catalogo='        <STRONG>'.$pmo[9].': </STRONG>'.initialize_download('produtos/catalogo/'.$prod[0][12]).'<br>';
else:
	$catalogo='';
endif;
if($prod[0][3]<>'' and $prod[0][3]<>'no_img.jpg'):
	$img=$upload_path.'/produtos/images/'.$prod[0][3];
	$full_img=$upload_path.'/produtos/images/original/'.$prod[0][3];
	$img='<a href="'.$full_img.'" target="_blank"><img alt="'.$prod[0][0].'" src="'.$img.'" border="0" width="100" /><br>'.$pmo[0].'</a>';
else:
	$img=$staticvars['site_path'].'/modules/produtos/images/no_img.jpg';
	$img='<img alt="'.$prod[0][0].'" src="'.$img.'" border="0" width="100" />';
endif;
// optimize 
$desconto=$db->getquery("select valor from produtos_desconto where cod_desconto='".$prod[0][5]."'");
$iva=$db->getquery("select valor,descricao from produtos_iva where cod_iva='".$prod[0][6]."'");
// optimize please!!!!
...
function search_products($staticvars){
...
if(isset($_POST['quick_search'])):
	...
	$res=$db->getquery("select titulo, descricao, short_descricao, imagem, preco, cod_desconto, cod_iva, stock, prazo_entrega, ref_produto, cod_categoria,cod_produto, active from produtos");
elseif(isset($_POST['adv_search'])):
	...
	$res=$db->getquery("select titulo, descricao, short_descricao, imagem, preco, cod_desconto, cod_iva, stock, prazo_entrega, ref_produto, cod_categoria,cod_produto, active from produtos ".$strings);	
endif;
if ($res[0][0]<>''):
...
function general_product_listing($staticvars,$pmg) {
include($staticvars['local_root'].'kernel/staticvars.php');
if(isset($_GET['orderby'])):
	if ($_GET['orderby']=='asc'):
		$orderby='ASC';
		$order_img=$staticvars['site_path'].'/modules/produtos/images/sort_asc.png';
		$order_href=strip_address('orderby',$_SERVER['REQUEST_URI']).'&orderby=ASC';
		$order_txt=$pmg[2];
	else:
		$orderby='DESC';
		$order_img=$staticvars['site_path'].'/modules/produtos/images/sort_desc.png';
...
if(isset($_POST['quick_search'])):
	...
	$header='  <h3><img src="'.$staticvars['site_path'].'/modules/produtos/images/icon.gif" width="37" height="34" align="absbottom" />'.$pmg[0].'</h3>';
elseif(isset($_POST['adv_search'])):
	$prod=search_products($staticvars['local_root']);
	$header='  <h3><img src="'.$staticvars['site_path'].'/modules/produtos/images/icon.gif" width="37" height="34" align="absbottom" />'.$pmg[0].'</h3>';
elseif(isset($_GET['cat'])):
	$header='  <h3><img src="'.$staticvars['site_path'].'/modules/produtos/images/icon.gif" width="37" height="34" align="absbottom" />'.$pmg[1].'</h3>';
	$prod=$db->getquery("select titulo, descricao, short_descricao, imagem, preco, cod_desconto, cod_iva, stock, prazo_entrega, ref_produto, cod_categoria,cod_produto, active from produtos where active='s' and cod_categoria='".mysql_escape_string($_GET['cat'])."'".$order);
else:
	$header='  <h3><img src="'.$staticvars['site_path'].'/modules/produtos/images/icon.gif" width="37" height="34" align="absbottom" />'.$pmg[1].'</h3>';
	$prod=$db->getquery("select titulo, descricao, short_descricao, imagem, preco, cod_desconto, cod_iva, stock, prazo_entrega, ref_produto, cod_categoria,cod_produto, active from produtos where active='s' ".$order);
endif;
...
<?
	...
	$desconto=$db->getquery("select cod_desconto, valor from produtos_desconto");
	for($i=0;$i<count($desconto);$i++):
		$descontos[$i]=$desconto[$i][0];
		$desc_valor[$i]=$desconto[$i][1];
	endfor;
	$descontos[]='end';
	$iva=$db->getquery("select cod_iva,descricao from produtos_iva");
	...
	for ($i=($lower-1);$i<=$up;$i++):
		if($prod[$i][3]<>'' and $prod[$i][3]<>'no_img.jpg'):
			$img=$upload_path.'/produtos/images/'.$prod[$i][3];
			$full_img=$upload_path.'/produtos/images/original/'.$prod[$i][3];
		...
		<table width="100%" border="0">
		  <tr>
			...
			<td width="91%"><strong><a style="FONT-WEIGHT: bold; FONT-SIZE: 16px" href="<?=session($staticvars,'index.php?id='.return_id('produtos_main.php').'&cod='.$prod[$i][11]);?>"><?=normalize_chars($prod[$i][0]);?></a></strong></td>
		  </tr>
		  <tr>
			<td align="justify"><?=normalize_chars($prod[$i][2]);?></td>
		  </tr>
		  <tr>
			<td align="justify"><a style="color:#000000" href="<?=session($staticvars,'index.php?id='.return_id('produtos_main.php').'&cod='.$prod[$i][11]);?>"><?=$pmg[6];?></a></td>
		  </tr>
		  <tr>
