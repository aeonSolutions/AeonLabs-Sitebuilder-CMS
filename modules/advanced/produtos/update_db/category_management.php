...
if (isset($_POST['estado'])): // activar 
	if ($_POST['cat_code']=='publish'):
		$db->setquery("update produtos_categorias set active='s' where cod_categoria='".$cat."'");
		echo '<font class="body_text"> <font color="#FF0000">Categoria publicada</font></font>';
	elseif($_POST['cat_code']=='unpublish'): // desactivar
		$db->setquery("update produtos_categorias set active='n' where cod_categoria='".$cat."'");
...
elseif (isset($_POST['apagar'])): // apagar 
	$cod_categoria=$db->getquery("select cod_sub_cat from produtos_categorias where cod_categoria='".$cat."'");
	$cod_categoria=$cod_categoria[0][0];
	$db->setquery("delete from produtos_categorias where cod_categoria='".$cat."'");
	$db->setquery("delete from produtos_categorias where cod_sub_cat='".$cat."'");
	echo '<font class="body_text"> <font color="#FF0000">Categoria apagada</font></font>';
	unset($_POST['del_cat']);
elseif (isset($_POST['cat_name']))://editar
		$db->setquery("update produtos_categorias set descricao='pt=".mysql_escape_string($_POST['cat_name'])."',
...
elseif (isset($_POST['add_cat_name']))://adicionar
		$db->setquery("insert into produtos_categorias set descricao='pt=".mysql_escape_string($_POST['add_cat_name'])."',
