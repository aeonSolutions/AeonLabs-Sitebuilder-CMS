...
function load_txtf($text){
	...
	while ($ok):
		...
		if ($ok==true):
			...
			$text = substr_replace($text, "", $init_pos,5);
			$tmp2=strpos($text,"[/url]");
			$text = substr_replace($text, "".$get_text, $final_pos,6);
		endif;
	endwhile;
	...
	while ($ok):
		...
		if ($ok==true):
			...
			$text = substr_replace($text, '[urls]', $init_pos,$final_pos-$init_pos+1);
			$init_pos=strpos($text,'[urls]');
			$final_pos=strpos($text,"[/url]",$init_pos);
			$get_text_url=substr($text,$init_pos+6,($final_pos)-($init_pos+6));
			$text =substr_replace($text,$get_text_url.'',$init_pos,$final_pos+5-$init_pos+1);
		endif;
	endwhile;
