...
if (!include($staticvars['local_root'].'general/site_handler.php')):
	echo 'Error: Security Not Found( mainpage)';
	exit;
endif;
...
if (isset($_POST['users_edit']) or isset($_POST['users_email'])):
	...
	if (isset($_POST['users_edit'])):
		...
		include($staticvars['local_root'].'modules/authoring/admin/edit.php');
	else:
		$_SESSION['users_email']=$_POST['cod_user'];
		include($staticvars['local_root'].'modules/authoring/admin/email_user.php');
	endif;
else:
	...
	for ($i=0;$i<count($delta);$i++):
		...
		if ($i==$alfa):
			...
			$sr .= '<a href="'.$address.'&alfa='.$i.'">'.$tmp.'</a>&nbsp;&nbsp;';
	endfor;
	?>
	...
	<table width="100%"  border="0" cellspacing="0" cellpadding="0">
		...
		<tr>
				<td>
				  <FORM class="form" action="<?=$address;?>" method="post" encType="multipart/form-data">
				  <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
					<TBODY>
					  ...
					  <TR>
						<TD align="left"><INPUT class="text" maxLength="25" size="60" name="search_query">&nbsp;&nbsp;
						<INPUT class="button" type="submit" value="Procurar" name="search_user"></TD>
					  </TR>
					  <TR>
						<TD align="left" height="5"></TD>
					  </TR>
					  <tr>
						<TD align="center"><INPUT class="button" type="submit" value="Utilizadores Desactivados" name="users_inactive">
						&nbsp;&nbsp;
						<INPUT class="button" type="submit" value="Utilizadores sem acesso &agrave; 6 meses" name="users_old"></TD>
					  </TR>
					  <TR>
					   <TD align="left">&nbsp;</td> 
					 </TR>
			  ...
			  <tr>
				<td class="Body_text" align="center">
				  <FORM name="form" action="<?=$address;?>" method="post" encType="multipart/form-data">
				  <TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
					<TBODY>
					  <TR>
				...
				if ($alfa<>'none'):
					if ( (isset($_POST['search_query']) and $_POST['search_query']<>'') or (isset($_GET['search']) and $_GET['search']<>'')):
						if (isset($_POST['search_query']) and $_POST['search_query']<>''):
							$query=finder($_POST['search_query']);
							$to_do='&search='.$_POST['search_query'];
						else:
							$query=finder($_GET['search']);
					...
					if ($query[0][0]<>''):
						...
						put_previous_next_page($lower,$upper,$total,$address.'&alfa='.$alfa);
						for ($i=$lower;$i<=$upper;$i++):
							...
							echo'</td></tr><tr><td><form class="form" enctype="multipart/form-data" action="'.$address.'&alfa='.$alfa.'" method="post">';
						...
						echo '</td></tr></table>';
						put_previous_next_page($lower,$upper,$total,$address.'&alfa='.$alfa);
						echo'<form class="form" enctype="multipart/form-data" action="'.$address.'&alfa='.$alfa.'" method="post">';
						echo '<div align="right"><input class="button" name="users_del_all" type="submit" value=" Apagar Todos ">';
						echo '</form>';
					else:
						if (isset($_POST['search_query']) and $_POST['search_query']<>''):
