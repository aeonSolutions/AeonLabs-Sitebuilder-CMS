...
$cod_categoria=0;
if (isset($_POST['cat'])):
	$cod_categoria=mysql_escape_string($_POST['cat']);
elseif(isset($_GET['cat'])):
	$cod_categoria=mysql_escape_string($_GET['cat']);
elseif(isset($_GET['mod'])):
	$cod_categoria=mysql_escape_string($_GET['mod']);
endif;
if(isset($_POST['cat_name']) or isset($_POST['add_cat_name']) or isset($_POST['cat_code']) or isset($_POST['del_cat'])):
	include($staticvars['local_root'].'kernel/staticvars.php');
	include($staticvars['local_root'].'modules/formacao/update_db/category_management.php');
endif;
$query=$db->getquery("select cod_categoria, nome from formacao_categorias where cod_categoria='".$cod_categoria."'");
if ($query[0][0]<>''):
...
<img src="<?=$staticvars['site_path'].'/modules/formacao';?>/images/icone_gestao.gif" /><font class="Header_text_4">Manuten&ccedil;&atilde;o de Categorias de Produtos</font><br />
<br />
<TABLE width="100%" height="500" border="0" cellPadding="0" cellSpacing="0">
  <tr valign="top" height="10">
	<td align="center">
		...
		<hr class="gradient">
		<font class="body_text">Ver sub-categorias existentes em <?=$cat_name;?>:&nbsp;</font><select size="1" name="cat" class="form_input">
			<?php
			$query=$db->getquery("select cod_categoria, nome from formacao_categorias where cod_sub_cat='".$cod_categoria."'");
			$option[0][0]='';
			$option[0][1]='-----------------';
			if($query[0][0]<>''):
		...
		<br />
		<a class="body_text" href="<?=$_SERVER['REQUEST_URI'].'&type=add&cat='.$cod_categoria;?>" >Adicionar Nova categoria</a>
		<hr class="gradient">
		</form>
	</td>
  </tr>
  ...
  <tr valign="top" height="100%">
	...
	<?php
	if ($cod_categoria<>''):
		$query=$db->getquery("select cod_categoria from formacao_categorias where cod_categoria='".$cod_categoria."'");
		if (isset($_GET['type'])):
			if($_GET['type']=='add'):
				add_field($staticvars,$cod_categoria);
				put_files($staticvars,$cod_categoria);
			else:
...
$query_a=$db->getquery("select cod_categoria, active,nome,descricao from formacao_categorias where cod_categoria='".$mod."'");
if($query_a[0][1]=='s'):
	$pub='Sim';
	$name='unpublish';
	$value='nao_publicar';
...
<table border="0" cellspacing="0">
  <tr>
	...
	<form method="post" action="<?=$address.'&mod='.$mod;?>"  enctype="multipart/form-data">
      <table border="0" cellpadding="0" cellspacing="0" align="center">
        ...
        <tr>
          <td colspan="2"><font class="body_text">Nome</font>&nbsp;&nbsp;&nbsp;
              <input type="text" name="cat_name" value="<?=$query_a[0][2];?>" maxlength="255" size="40" /></td>
        </tr>
        <tr>
          <td height="15"></td>
        </tr>
        <tr>
          <td colspan="2"><font class="body_text">Descri&ccedil;&atilde;o</font>&nbsp;&nbsp;&nbsp;
	  ...
	  <tr>
        ...
        <tr>
          <td align="right" valign="bottom"><input name="add_sub_menu2" type="submit" value="Gravar" class="form_submit" />          </td>
        </tr>
      </table>
	  </form>	</td>
  </tr>
  ...
  <input name="apagar2" type="submit" class="form_submit" value="<?=$name;?>"/>
      </form></td>
    <td align="left" valign="top"><form method="post" action="<?=$address.'&mod='.$mod;?>" target="_parent">
  &nbsp;&nbsp;
	...
	<table border="0" cellpadding="0" cellspacing="0" align="center">
	  <tr>
		<td colspan="2">
			...
			<input type="text" name="add_cat_name" maxlength="255" value="" size="40">		</td>
	  </tr>
	  <tr>
		<td height="5" colspan="2"></td>
	  </tr>
	  <tr>
		<td colspan="2">
		    ...
		    <input type="text" name="add_cat_disp_name" maxlength="255" value="" size="40">
		<br />
		ex.: en=calendar||pt=calend&aacute;rio</td>
	  </tr>
        <tr>
	  ...
	  <tr>
		<td align="right">
		  <input name="add_sub_menu" type="submit" value="Adicionar" class="form_submit"></td>
	  </tr>
	  </table>
</form>
...
<table border="0" height="50" align="center">
  <tr>
	...
	if ($mod<>''):
		...
		$i=0;
		$res[0][0]='';
		$query=$db->getquery("select cod_categoria, nome, cod_sub_cat from formacao_categorias where cod_categoria=".$ct);
    ...
    <td style="BORDER-RIGHT: lightsteelblue 1px solid; BORDER-TOP: lightsteelblue 1px solid; MARGIN: 0px; OVERFLOW: hidden; BORDER-LEFT: lightsteelblue 1px solid; BORDER-BOTTOM: lightsteelblue 1px solid;">
	<?php
	$query=$db->getquery("select cod_categoria, nome from formacao_categorias where cod_sub_cat='".$mod."' order by nome");
	if ($query[0][0]==''):
		?>
		...
		while ($i<count($query)):
			...
			<tr>
			<?php
			$query2=$db->getquery("select cod_categoria, nome from formacao_categorias where cod_sub_cat='".$query[$i][0]."' order by nome asc limit 0,7");
			if ($query2[0][0]<>''):
				?>
			...
			if (isset($query[$i+1])):
				$query3=$db->getquery("select cod_categoria, nome from formacao_categorias where cod_sub_cat='".$query[$i+1][0]."' order by nome asc limit 0,7");
				if ($query3[0][0]<>''):
					?>
					<td align="center" valign="top" width="50%"><div align="left">
					<?php
