...
if (isset($_POST['estado'])): // activar 
	if ($_POST['cat_code']=='Activar'):
		$db->setquery("update publicacoes_categorias set active='s' where cod_categoria='".$cat."'");
		echo '<font class="body_text"> <font color="#FF0000">Categoria publicada</font></font>';
	elseif($_POST['cat_code']=='Desactivar'): // desactivar
		$db->setquery("update publicacoes_categorias set active='n' where cod_categoria='".$cat."'");
...
elseif (isset($_POST['apagar'])): // apagar 
	$db->setquery("update publicacoes_categorias set cod_sub_cat='0' where cod_sub_cat='".$cat."'");
	$db->setquery("delete from publicacoes_categorias where cod_categoria='".$cat."'");
	echo '<font class="body_text"> <font color="#FF0000">Categoria apagada</font></font>';
	unset($_POST['del_cat']);
elseif (isset($_POST['cat_name']))://editar
		$db->setquery("update publicacoes_categorias set descricao='".mysql_escape_string($_POST['cat_name'])."',
...
elseif (isset($_POST['add_cat_name']))://adicionar
		$db->setquery("insert into publicacoes_categorias set descricao='".mysql_escape_string($_POST['add_cat_name'])."',
