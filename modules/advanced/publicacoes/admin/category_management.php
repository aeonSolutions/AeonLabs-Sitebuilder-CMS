...
if(isset($_POST['cat_name']) or isset($_POST['add_cat_name']) or isset($_POST['cat_code']) or isset($_POST['del_cat'])):
	...
	include($staticvars['local_root'].'modules/publicacoes/update_db/category_management.php');
endif;
$cat_name[0]='';
$query=$db->getquery("select cod_categoria, nome, cod_sub_cat from publicacoes_categorias where cod_categoria='".$cod_categoria."'");
if ($query[0][0]<>''):
	$cat_name[0]='<a href="'.strip_address('cat',$_SERVER['REQUEST_URI']).'&cat='.$query[0][0].'">'.$query[0][1].'</a>';
	$k=0;
	while($query[0][2]<>0):
		$query=$db->getquery("select cod_categoria, nome, cod_sub_cat from publicacoes_categorias where cod_categoria='".$query[0][2]."'");
		$k++;
...
<br />
<h3><img src="<?=$staticvars['site_path'].'/modules/publicacoes';?>/images/icone_gestao.gif" width="24" height="24" />Manuten&ccedil;&atilde;o de Categorias de publicacoes</h3>
<br />
<br />
<TABLE width="100%" height="500" border="0" cellPadding="0" cellSpacing="0">
  <tr valign="top" height="10">
	<td align="center">
		<form class="form" method="post" action="<?=strip_address("type",$_SERVER['REQUEST_URI']);?>" enctype="multipart/form-data">
		...
		<font class="body_text">Ver Categorias existentes:&nbsp;</font><select size="1" name="cat" class="text">
			<?php
			$query=$db->getquery("select cod_categoria, nome from publicacoes_categorias where cod_sub_cat='".mysql_escape_string($cod_categoria)."'");
			$option[0][0]='';
			$option[0][1]='-----------------';
			if($query[0][0]<>''):
		...
		<br />
		<br />
		<img src="<?=$staticvars['site_path'];?>/modules/publicacoes/images/add.gif" width="24" height="22" /><a class="body_text" href="<?=strip_address("type",strip_address("cat",$_SERVER['REQUEST_URI'])).'&type=add&cat='.$cod_categoria;?>" >Adicionar nova categoria dentro desta categoria</a>
		<hr class="gradient">
		</form>
	</td>
  </tr>
...
$query_a=$db->getquery("select cod_categoria, active,nome,descricao from publicacoes_categorias where cod_categoria='".$mod."'");
if($query_a[0][1]=='s'):
	$pub='Sim';
	$name='Desactivar';
	$value='nao_publicar';
...
include($staticvars['local_root'].'kernel/staticvars.php');
$address=str_replace("&mod=".$mod,"",$_SERVER['REQUEST_URI']);
$query=$db->getquery("select cod_categoria, nome, cod_sub_cat from publicacoes_categorias where cod_categoria='".$mod."'");
if ($query[0][0]==''):
	$query[0][0]='';
	$query[0][1]='raiz';
endif;
...
<table border="0" height="50" align="center">
  <tr>
	...
	if ($mod<>''):
		...
		$i=0;
		$res[0][0]='';
		$query=$db->getquery("select cod_categoria, nome, cod_sub_cat from publicacoes_categorias where cod_categoria=".$ct);
    ...
    <td style="BORDER-RIGHT: lightsteelblue 1px solid; BORDER-TOP: lightsteelblue 1px solid; MARGIN: 0px; OVERFLOW: hidden; BORDER-LEFT: lightsteelblue 1px solid; BORDER-BOTTOM: lightsteelblue 1px solid;">
	<?php
	$query=$db->getquery("select cod_categoria, nome from publicacoes_categorias where cod_sub_cat='".$mod."' order by nome");
	if ($query[0][0]==''):
		?>
		...
		while ($i<count($query)):
			...
			<tr>
			<?php
			$query2=$db->getquery("select cod_categoria, nome from publicacoes_categorias where cod_sub_cat='".$query[$i][0]."' order by nome asc limit 0,7");
			if ($query2[0][0]<>''):
				?>
			...
			if (isset($query[$i+1])):
				$query3=$db->getquery("select cod_categoria, nome from publicacoes_categorias where cod_sub_cat='".$query[$i+1][0]."' order by nome asc limit 0,7");
				if ($query3[0][0]<>''):
					?>
					<td align="center" valign="top" width="50%"><div align="left">
					<?php
