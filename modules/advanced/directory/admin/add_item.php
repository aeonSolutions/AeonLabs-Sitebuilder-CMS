...
if (!include($staticvars['local_root'].'general/site_handler.php')):
	echo 'Error: Security Not Found (add item)';
	exit;
endif;
if (isset($_GET['id'])):
	$task=$_GET['id'];
...
if(!isset($_POST['add_item_p1']) and !isset($_POST['add_item_p3']) and !isset($_GET['cod']) or $type=='' or $step==1):
	include($staticvars['local_root'].'modules/directory/admin/add_item_p1.php');
...
elseif((isset($_POST['add_item_p3']) or $enable_directory==false or $step==3) and !isset($_POST['titulo'])):
		$message='';
		include($staticvars['local_root'].'modules/directory/admin/add_item_p3.php');
elseif(isset($_POST['titulo'])):
	if ($_POST['titulo']<>'' ):
		include($staticvars['local_root']."/modules/directory/update_db/manage.php");
		if ($message==''):
			include($staticvars['local_root'].'modules/directory/update_db/add_items_send_mail.php');
			include($staticvars['local_root'].'modules/directory/admin/add_item_success.php');
		else:
			include($staticvars['local_root'].'modules/directory/admin/add_item_p3.php');
		endif;
	else:
