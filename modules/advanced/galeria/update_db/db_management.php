...
if (isset($_FILES['imagem'])):
	if (stristr($_FILES['imagem']['type'],"jpeg")or stristr($_FILES['imagem']['type'],"gif")):
			...
			if (check_files($upload_directory.'/galeria/images',$name)):
				$tmp=explode(".",$name);
				$tmp[0].=generate('5','No','Yes','No');
				$tmp1[1]=$tmp[0].'.'.$tmp[1];
				$location_original=$upload_directory.'/galeria/images/original/'.$tmp1[1];
				$location=$upload_directory.'/galeria/images/'.$tmp1[1];
			else:
				$location_original=$upload_directory.'/galeria/images/original/'.$name;					
				$location=$upload_directory.'/galeria/images/'.$name;					
				$tmp1[1]=$name;
			endif;
...
if (isset($_FILES['catalogo'])):
	if (stristr($_FILES['catalogo']['type'],"pdf")or stristr($_FILES['catalogo']['type'],"zip")):
			...
			if (check_files($upload_directory.'/galeria/catalogo',$name)):
				$tmp=explode(".",$name);
				$tmp[0].=generate('5','No','Yes','No');
				$tmp1[1]=$tmp[0].'.'.$tmp[1];
				$location=$upload_directory.'/galeria/catalogo/'.$tmp1[1];
			else:
				$location=$upload_directory.'/galeria/catalogo/'.$name;					
				$tmp1[1]=$name;
			endif;
...
elseif (isset($_POST['edit_product']) and $message=='')://editar box
	...
	if($_POST['titulo']<>'' and $_POST['descricao']<>'' and $_POST['descricao2']<>'' and $_POST['preco']<>'' and $_POST['stock']<>'' and $_POST['prazo_entrega']<>''
	 and $_POST['ref_produto']<>'' and $_POST['categoria']<>'0' and $_POST['iva']<>'0' and $_POST['desconto']<>'null' ):
		$query="update galeria set titulo='".normalize_chars(stripslashes(mysql_escape_string($_POST['titulo'])))."', descricao='".normalize_chars(stripslashes(mysql_escape_string($_POST['descricao'])))."',
		...
		if ($image<>'no_img.jpg'):
		 ...
		 if ($catalogo=='no_cat'):
			$query.=$active." where cod_galeria='".$bc."'";
		else:
			$query.=", catalogo='".mysql_escape_string($catalogo)."'".$active." where cod_galeria='".$bc."'";
		endif;
		$db->setquery($query);
...
elseif (isset($_POST['add_product']) and $message=='')://inserir box
	...
	if($_POST['titulo']<>'' and $_POST['descricao']<>'' and $_POST['descricao2']<>'' and $_POST['preco']<>'' and $_POST['stock']<>'' and $_POST['prazo_entrega']<>''
	 and $_POST['ref_produto']<>'' and $_POST['categoria']<>'0' and $_POST['iva']<>'0' and $_POST['desconto']<>'null' ):
		$db->setquery("insert into galeria set titulo='".normalize_chars(stripslashes(mysql_escape_string($_POST['titulo'])))."', descricao='".normalize_chars(stripslashes(mysql_escape_string($_POST['descricao'])))."',
		short_descricao='".normalize_chars(stripslashes(mysql_escape_string($_POST['descricao2'])))."', preco='".mysql_escape_string($_POST['preco'])."',
		cod_iva='".mysql_escape_string($_POST['iva'])."', cod_desconto='".mysql_escape_string($_POST['desconto'])."',
...
if ($message<>''):
	$_SESSION['update']='galeria:'.$message;
endif;
?>