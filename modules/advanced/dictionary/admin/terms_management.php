...
<TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
  <TBODY>
    <TR>
      <TD colspan="2" vAlign=top class="header-bk"><H3>Gest&atilde;o de termos no dicion&aacute;rio</H3></TD>
    </TR>
    <TR>
...
include($staticvars['local_root'].'kernel/staticvars.php');
$query4=$db->getquery("select cod_dic, termo, definicao,email from dictionary where active='?'");
$address=strip_address("cod",$_SERVER['REQUEST_URI']);
if (@$query4[0][1]<>''):
	?>
	...
	for ($i=($lower-1);$i<=$up;$i++):
		...
		<tr>
		  <td align="left">
		  <font class="body_text">&nbsp;&nbsp;Palavra: <b><?=$query4[$i][1];?></b><br>
		  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?=$query4[$i][2];?><br></font>
		  </td>
		  <td align="center">
	...
	echo 'n&atilde;o existem termos a publicar';
endif;
};
...
$email_options='';
if(isset($_GET['cod'])):
	$term=$db->getquery("select termo, definicao, active, imagem, email from dictionary where cod_dic='".mysql_escape_string($_GET['cod'])."'");
	if($term[0][0]<>''):
		if($term[0][2]=='s'):
		...
		$term[0][4]='';
		$term[0][5]='';
		$add_buttons='<input type="submit" name="add_term" id="add_term" value="Submeter palavra" class="form_submit">';
	endif;
else:
	$term[0][1]='';
	$term[0][2]='';
