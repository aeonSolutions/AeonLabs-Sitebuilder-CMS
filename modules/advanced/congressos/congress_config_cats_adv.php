...
if((isset($_POST['gravar_novo']) and $_POST['gravar_novo']<>'') or (isset($_POST['gravar_detalhes']) and $_POST['gravar_detalhes']<>'') or isset($_POST['del_pages']) or isset($_POST['elm1'])):
	...
	session_write_close();
	sleep(1);
	header("Location: ".strip_address("cat",strip_address("see",$_SERVER['REQUEST_URI'])));
endif;
...
if (isset($_POST['editar_file'])):
	...
	if($cat[0][0]<>''):
		if(is_file($staticvars['local_root'].'modules/congressos/contents/'.$cat[0][2].'/'.$_GET['lang'].'/main.php')):
			$code=file_get_contents($staticvars['local_root'].'modules/congressos/contents/'.$cat[0][2].'/'.$_GET['lang'].'/main.php');
			...
			<td>
				...
				if ($cat[0][0]<>''):
					...
					for($i=0; $i<count($cat);$i++):
						echo '<li class="cat-item"><a href="'.strip_address("load",strip_address("cat",$_SERVER['REQUEST_URI'])).'&cat='.$cat[$i][0].'"></li>'.$cat[$i][1].'</a>';
					endfor;
					echo '</ul></div>';
				else:
			...
			if(isset($_GET['cat'])):
				...
				if($cat[0][0]<>''):
					if(isset($_GET['contents']) or isset($_POST['edit_contents'])):// edit contents
						include($staticvars['local_root'].'modules/congressos/system/edit_contents.php');	
					else:
