...
*/
?>
<SCRIPT language="JavaScript" src="<?=$site_path;?>/setup/layout/menu/layouts/horz_fire/menu_builder.js"></SCRIPT>
<?php
...
$tree=build_menu($session_users);
$menu_name='';
if (count($tree)<>0):
	include_once($local_root.'general/pass_generator.php');
	include_once($local_root.'general/staticvars.php');
	$k=0;
	$menu='';
	for($i=0; $i<count($tree); $i++):
		...
		elseif($tree[$i]['flag']=='option'):
			$menu .= $menu_name[$k-1]['name'].'.addMenuItem("'.normalize_chars($tree[$i]['name']).'","location=\''.$site_path.'/'.$tree[$i]['link'].'\'");';
			$menu.='
			';
		endif;
	endfor;
		...
		if ($menu_name<>''):
			for($i=0; $i<count($menu_name); $i++):
				...
				if ($menu_name[$i]['type']=='menu'):
					...
					onmouseout=MM_startTimeout(); name="'.$menu_name[$i]['id'].'">
					<img style="display:inline" src="'.$site_path.'/layout/menu/layouts/horz_fire/seta.gif" width="12" height="12" border="0">&nbsp;'.$tree[$menu_name[$i]['pos']]['name'].'&nbsp;&nbsp;&nbsp;</a>
					';
				else:
					echo '<a class="menu_links" id="'.$menu_name[$i]['id'].'" href="'.$tree[$menu_name[$i]['pos']]['link'].'" target="_top">
					<img style="display:inline" src="'.$site_path.'/layout/menu/layouts/horz_fire/seta.gif" width="12" height="12" border="0">&nbsp;'.$tree[$menu_name[$i]['pos']]['name'].'&nbsp;&nbsp;&nbsp;</a>
